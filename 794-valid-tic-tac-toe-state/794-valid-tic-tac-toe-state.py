class Solution:
    ...
    def isSolved(self, board, ch):
        if max([sum([1 if board[i][j] == ch else 0 for j in range(3)]) for i in range(3)]) == 3 or max([sum([1 if board[i][j] == ch else 0 for i in range(3)]) for j in range(3)]) == 3 or sum([1 if board[i][i] == ch else 0 for i in range(3)]) == 3 or sum([1 if board[2 - i][i] == ch else 0 for i in range(3)]) == 3:
            return True
        return False